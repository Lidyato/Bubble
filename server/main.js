...
function userLogin (request, response){
	response.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8' });
		...
		++userID;
		response.end(JSON.stringify({id: ++usersCounter}));
};

// User Logout case
function userLogout (request, response){
	response.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8' });
	usersCounter = usersCounter > 0 ? --usersCounter : usersCounter;
	while (Babble.messagesClients.length > 0) {
		var client = Babble.messagesClients.pop();
		client.response.end(JSON.stringify([ ]));
	}
    ...
    request.on('end', function () {
        ...
        https.get(options, function (res) {
            ...
            res.on('end', function () {
                var gravatarUrl = '';
                try {
                    gravatarUrl = JSON.parse(body).entry[0].thumbnailUrl;
                } catch (err) {
                    gravatarUrl = '';
                }
...
function getStatsReq (response){
	response.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8' });
	console.log("users from get stats = " + usersCounter);
	response.end(JSON.stringify({users: usersCounter, messages: messagesUtils.getMessagesCount()}));
};
